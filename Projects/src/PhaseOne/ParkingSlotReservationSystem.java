 ...
 * Students' names:
 * 1. Fahad Almutairi. {case 2} TIP: THIS PART HAS A TRICK PAY ATTENTION TO LINE(256) ദ്ദി（• ˕ •マ.ᐟ !
					...
					if (choice == 1) {
						...
						do {
							...
							if (tempDurationChoice >= 1 && tempDurationChoice <= 4) {
								tempcCost = tempDurationChoice * ONE_HOUR_PRICE;
								isSlotselected = true;
							} else if (tempDurationChoice == 5) {
								tempcCost = ONE_DAY_PRICE;
								isSlotselected = true;
							} else {
						...
						if (!isSlotselected) {
							...
							System.out.println(String.format("Total Cost      : %.2f SAR", tempcCost));
							int confirm = 0;
							do {
								...
								if (confirm == 1) {
									...
									System.out.println(
											String.format("Reservation cancelled.\n Returning to Main Menu..."));
									choice = 3;
								} else {
				...
				do {
					...
					if (adminChoice == 1) {
						...
						if (maxCount == 0) {
							...
							System.out.println();
							System.out.println(String.format("Number of Bookings: %d times", maxCount));
						}
